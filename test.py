...
img_path = "/home/andrew/computer vision/dataset/sequences/00/image_0/"
pose_path = "/home/andrew/computer vision/dataset/poses/00.txt"
...
while(vo.hasNextFrame()):
    ...
    print("MSE Error: ", np.linalg.norm(np.array((mono_coord[0], mono_coord[2])) - np.array((true_coord[0], true_coord[2]))))
    print("x: {}, y: {}, z: {}".format(*[str(pt) for pt in mono_coord]))
    print("true_x: {}, true_y: {}, true_z: {}".format(*[str(pt) for pt in true_coord]))

    err.append(np.linalg.norm(np.array((mono_coord[0], mono_coord[2])) - np.array((true_coord[0], true_coord[2]))))
    ...
    cv.putText(traj, 'Average MSE:', (155, 150), cv.FONT_HERSHEY_SIMPLEX, 0.5,(255,255,255), 1)
    cv.putText(traj, prev, (270, 150), cv.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 0), 1)
    cv.putText(traj, str(np.round(sum(err)/len(err), 2)), (270, 150), cv.FONT_HERSHEY_SIMPLEX, 0.5,(255, 255, 255), 1)
    

    prev = str(np.round(sum(err)/len(err), 2))
    cv.imshow('trajectory', traj)
    image = cv.cvtColor(vo.result, cv.COLOR_BGR2RGB)
    img_list.append(image.astype(np.uint8))
    traj_list.append(traj.astype(np.uint8))
    if vo.id == 2500:
        #cv.imwrite("/home/andrew/computer vision/ComputerVision-OpticalFlow/VisualSLAM/images/observability.png", vo.result)
        imageio.mimsave('video.mp4', img_list, duration=1000/60)
        imageio.mimsave('traj.mp4', traj_list, duration=1000/60)
